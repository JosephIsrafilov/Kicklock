...
public:
    ...
    void runTest() override
    {
        ...
        beginTest ("Stable multi-hit loop aggregates to a confident, applicable fix");
        {
            ...
            const auto second = processor.analyzeFix();

            expectGreaterThan (first.contributingHits, 1);
            expect (! first.unstableRecommendation, first.message);
            // Deterministic across repeated presses on the same captured material.
            expectWithinAbsoluteError (second.bassDelayMs, first.bassDelayMs, 0.05f);
            expectEquals ((int) second.quality, (int) first.quality);
        }
